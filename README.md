...
When the separate **Linux Wallpaper Engine** graphical frontend is also configured to start at login, the service lets it restore the screens in its saved configuration. Other screens are restored immediately. Filesystem notifications confirm the frontend launches, without polling. If it has not restored a screen within 30 seconds, the service checks again and takes over missing screens. This avoids competing startup routines killing and restarting the same wallpaper; the frontend and its tray remain available.
